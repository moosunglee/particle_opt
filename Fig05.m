...
pitch = 0.05;
%%
sio2_mie_data = load(fullfile(current_dirname, 'mie_diameter_plots', 'Si_cbs_mie_rayleigh.mat'));

% Scan enhancement factors (freq & eta)
dim_list = [2,1,3];


datadir_cylinder = dir(fullfile(current_dirname, 'Si_cylinder', 'summary*.mat'));
...
for j0 = 1:3
    ...
    for j1 = 1:length(datadir_cylinder)
        ...
        scatter(data_opt.radius_mie_lists(1)*2, data_opt.Freqs_opt(dim_list(j0)), 150, 'cs', ...
...
for j0 = 1:3
    ...
    for j1 = 1:length(datadir_cylinder)
        data_opt = load(fullfile(datadir_cylinder(j1).folder,spnames{(j1)}));
        scatter(data_opt.radius_mie_lists(1)*2, data_opt.Etas_opt(dim_list(j0)), 150, 'cs', ...
          'MarkerEdgeColor','none',...
              'MarkerFaceColor',c(1+round((depth_list((j1))-min_depth)/(max_depth-min_depth)*255),:),...
            'LineWidth',1)
    end
...
for j0 = 1:3
    ...
    for j1 = 1:length(datadir_cylinder)
        data_opt = load(fullfile(datadir_cylinder(j1).folder,spnames{(j1)}));
        scatter(data_opt.radius_mie_lists*2, data_opt.Freqs_rot_opt(dim_list(j0)), 150, 'cs', ...
          'MarkerEdgeColor','none',...
              'MarkerFaceColor',c(1+round((depth_list((j1))-min_depth)/(max_depth-min_depth)*255),:),...
            'LineWidth',1)
    end
...
for j0 = 1:3
    ...
    for j1 = 1:length(datadir_cylinder)
        data_opt = load(fullfile(datadir_cylinder(j1).folder,spnames{(j1)}));
        scatter(data_opt.radius_mie_lists*2, data_opt.Etas_rot_opt(dim_list(j0)), 150, 'cs', ...
          'MarkerEdgeColor','none',...
              'MarkerFaceColor',c(1+round((depth_list((j1))-min_depth)/(max_depth-min_depth)*255),:),...
            'LineWidth',1), hold on
    end
